...
import { COLORS, FONT_FAMILIES, MEDIA_QUERIES } from "../components/constants"
...
export const Wrapper = styled.div`
  ...
  .description {
    ...
    background-color: ${({ descriptionColor = COLORS.PRIMARY }) =>
      `${descriptionColor}`};
    h2 {
      font-family: ${FONT_FAMILIES.TITLE};
      font-size: 3rem;
  ...
  .collectables {
    ...
    width: 100%;
    padding: 2rem 5%;
    background-color: ${({ collectablesColor = COLORS.BLACK }) =>
      `${collectablesColor}`};
    h2 {
      font-family: ${FONT_FAMILIES.TITLE};
      font-size: 3rem;
...
export const Collect = styled(Link)`
  display: flex;
  position: relative;
  width: 15vw;
  height: 15vw;
...
export const BottomEdgeDown = styled.div`
  ...
  border-right: 64.45vw  solid transparent;
  &:before {
    content: "";
    display: block;
...
export const BottomEdgeUp = styled.div`
  ...
  &:after {
    content: "";
    display: block;
    border-bottom: ${({ color }) => `140px solid ${color}`};
    border-left: 64.45vw solid transparent;
  ...
  @media (max-width: ${MEDIA_QUERIES.MEDIUM}) {
    ...
    &:after {
      border-bottom: ${({ color }) => `50px solid ${color}`};
